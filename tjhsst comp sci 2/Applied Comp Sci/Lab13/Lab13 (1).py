...
def draw_rectangle(event):
   ...
   if 100<event.x<800:
      ...
      if len(rect_clicks) == 2:
         cnvs.create_rectangle(rect_clicks[0][x],rect_clicks[0][y],rect_clicks[1][x],rect_clicks[1][y],fill=color)
         rect_clicks = []
#free draw      
def free_draw(event):
   if 100<event.x<800:
...
def spray_can(event):
   if 100<event.x<800:
      for i in range(50):
         ...
         cnvs.create_oval(pointx-1,pointy-1,pointx+1,pointy+1,fill=color)
#draw oval
def draw_oval(event):     
   global oval_clicks
   if 100<event.x<800:
      ...
      if len(oval_clicks) == 2:
         cnvs.create_oval(oval_clicks[0][x],oval_clicks[0][y],oval_clicks[1][x],oval_clicks[1][y],fill=color)
         oval_clicks = []
