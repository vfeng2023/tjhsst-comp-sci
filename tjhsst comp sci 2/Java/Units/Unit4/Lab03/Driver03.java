    ...
    public class Driver03
   {
       ...
       public static void main(String[] args) throws Exception
      {
         ...
         double[] temperaturesF = new double[NUMITEMS];
      	for(int i=0;i<NUMITEMS;i++){
            temperaturesF[i] = infile.nextDouble();
         }
      	
      	//process the array
         double[] tempsC = new double[NUMITEMS];
         for(int j=0;j<NUMITEMS;j++){
            double celcius = (temperaturesF[j]-32)*(5.0/9); //find celcius
            celcius = Math.floor(celcius*100)/100; //truncate to two decimal places
            tempsC[j] = celcius;
         }
      	//output
         ...
         for(int k=0;k<NUMITEMS;k++){
            System.out.println(temperaturesF[k]+"             | "+tempsC[k]);
